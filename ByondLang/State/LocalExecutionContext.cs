...
namespace ByondLang.State
{
    public class LocalExecutionContext : IExecutionContext
    {
        ...
        public void Dispose()
        {
            programService.FullDispose();
        }

        public async Task ExecuteScript(string script)
        {
            await programService.execute(new ExecuteRequest()
            {
                Code = script
            }, null);
        }
        ...
        public async Task<string> GetBuffer()
        {
            try
            {
                lastStatus = await programService.status(new StatusRequest(), null);
        ...
        {
            await programService.handleTopic(new TopicRequest()
            {
                TopicId = topic,
                Data = data
            }, null);
        ...
        public async Task InitializeProgram(ProgramType type)
        {
            lastStatus = await programService.status(new StatusRequest()
            {
                Initialize = true,
                Type = type,
            }, null);
        ...
        public Task<bool> Recycle()
        {
            programService.recycle(null, null);

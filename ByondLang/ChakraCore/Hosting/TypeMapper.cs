...
namespace ByondLang.ChakraCore.Hosting
{
	/// <summary>
	/// Type mapper
	/// Custumized version of https://github.com/Taritsyn/JavaScriptEngineSwitcher/blob/master/src/JavaScriptEngineSwitcher.ChakraCore/JsRt/TypeMapper.cs
	/// </summary>
	internal sealed class TypeMapper : IDisposable
	{
		...
		private EmbeddedType CreateEmbeddedType(Type type)
		{
			...
			JsNativeFunction nativeConstructorFunction = (callee, isConstructCall, args, argCount, callbackData) =>
			{
				...
				{
					CreateAndSetError($"Host type '{typeName}' has no contructors.");
					return undefinedValue;
				}
				...
				{
					CreateAndSetReferenceError($"Appropriate constructor for type '{typeName}' could not be located");
					return undefinedValue;
				}
				...
				catch (Exception e)
				{
					...
					JsValue errorValue = wrapperException != null ?
						CreateErrorFromWrapperException(wrapperException)
						:
						JsValue.CreateError(JsValue.FromString($"Contructor call for type '{typeName}' failed with exception: {exception.Message}"))
						;
					JsContext.SetException(errorValue);
		...
		private void ProjectFields(EmbeddedItem externalItem)
		{
			...
			foreach (FieldInfo field in fields)
			{
				...
				JsNativeFunction nativeGetFunction = (callee, isConstructCall, args, argCount, callbackData) =>
				{
					...
					{
						CreateAndSetTypeError($"Context error while invoking getter '{fieldName}'.");
						return undefinedValue;
					}
				...
				JsNativeFunction nativeSetFunction = (callee, isConstructCall, args, argCount, callbackData) =>
				{
					...
					if (instance && obj == null)
					{
						CreateAndSetTypeError($"Invalid context got host object field {fieldName}.");
						return undefinedValue;
					}
			...
			foreach (PropertyInfo property in properties)
			{
				...
				if (property.GetGetMethod() != null)
				{
					JsNativeFunction nativeGetFunction = (callee, isConstructCall, args, argCount, callbackData) =>
					{
						...
						if (instance && obj == null)
						{
							CreateAndSetTypeError($"Invalid context for '{propertyName}' property.");
							return undefinedValue;
						}
		...
		private void ProjectMethods(EmbeddedItem externalItem)
		{
			...
			foreach (IGrouping<string, MethodInfo> methodGroup in methodGroups)
			{
				...
				JsNativeFunction nativeFunction = (callee, isConstructCall, args, argCount, callbackData) =>
				{
					...
					if (instance && obj == null)
					{
						CreateAndSetTypeError($"Invalid context while calling method '{methodName}'.");
						return undefinedValue;
					}
					...
					{
						CreateAndSetReferenceError($"Suitable method '{methodName}' was not found.");
						return undefinedValue;
					}
